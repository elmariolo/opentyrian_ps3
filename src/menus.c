...
bool select_difficulty( void )
{
	...
	for (; ; )
	{
		...
		{
			if ((difficulty_max < 4 && isSafeKeyPressed(SDLK_g)) ||
			    (difficulty_max == 4 && isSafeKeyPressed(SDLK_RIGHTBRACKET)))
			{
				difficulty_max++;
			}
		} else if (difficulty_max == 5 && isSafeKeyPressed(SDLK_l) && isSafeKeyPressed(SDLK_o) && isSafeKeyPressed(SDLK_r) && isSafeKeyPressed(SDLK_d)) {
			difficulty_max++;
		}
