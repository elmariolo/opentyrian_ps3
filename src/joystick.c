...
void reset_joystick_assignments( int j )
{
	...
	for (uint a = 0; a < COUNTOF(joystick[j].assignment); a++)
	{
		...
		for (uint i = 0; i < COUNTOF(joystick[j].assignment[a]); i++)
		    ...
		    // --- MOVIMIENTO (DPAD) ---
			joystick[j].assignment[0][0].type = BUTTON; joystick[j].assignment[0][0].num = 3; // Arriba (Dpad-Arriba)
			joystick[j].assignment[1][0].type = BUTTON; joystick[j].assignment[1][0].num = 2; // Derecha (Dpad-Derecha)
			joystick[j].assignment[2][0].type = BUTTON; joystick[j].assignment[2][0].num = 1; // Abajo (Dpad-Abajo)
			joystick[j].assignment[3][0].type = BUTTON; joystick[j].assignment[3][0].num = 0; // Izquierda (Dpad-Izquierda)

			// --- ACCIONES DE COMBATE ---
			// Fuego Principal -> Cross (9)
			joystick[j].assignment[4][0].type = BUTTON; joystick[j].assignment[4][0].num = 9; 
			
			// Cambiar Fuego -> Circle (10)
			joystick[j].assignment[5][0].type = BUTTON; joystick[j].assignment[5][0].num = 10;

			// Sidekick Izquierdo -> L1 (13)
			joystick[j].assignment[6][0].type = BUTTON; joystick[j].assignment[6][0].num = 13;

			// Sidekick Derecho -> R1 (12)
			joystick[j].assignment[7][0].type = BUTTON; joystick[j].assignment[7][0].num = 12;

			// --- MENÚS Y PAUSA ---
			// Menú (Tyrian usa esto para salir o configuración) -> Select (7)
			joystick[j].assignment[8][0].type = BUTTON; joystick[j].assignment[8][0].num = 7;

			// Pausa (Y navegar en menús) -> Start (4)
			joystick[j].assignment[9][0].type = BUTTON; joystick[j].assignment[9][0].num = 4;
			
			// --- MAPEO DE STICK ANALÓGICO (Izquierdo) ---
			// Arriba/Abajo (Eje 1)
			joystick[j].assignment[0][1].type = AXIS;
			joystick[j].assignment[0][1].num = 1;
			joystick[j].assignment[0][1].negative_axis = true; // Arriba es negativo

			joystick[j].assignment[2][1].type = AXIS;
			joystick[j].assignment[2][1].num = 1;
			joystick[j].assignment[2][1].negative_axis = false; // Abajo es positivo

			// Izquierda/Derecha (Eje 0)
			joystick[j].assignment[3][1].type = AXIS;
			joystick[j].assignment[3][1].num = 0;
			joystick[j].assignment[3][1].negative_axis = true; // Izquierda es negativo

			joystick[j].assignment[1][1].type = AXIS;
			joystick[j].assignment[1][1].num = 0;
			joystick[j].assignment[1][1].negative_axis = false; // Derecha es positivo
	}
